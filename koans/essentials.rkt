...
;; define can be used to create functions (Or "procedures", to follow the
;; docs). The first identifier in the list after `define` is the procedure's
;; name.
...
;; the test-expr is evaluated first, and then only the relevant expression
;; evaluates afterwards. The entire `if` form returns the result of the
;; matching expression.
;;
;; We will cover more special rules as we go. The fundamental point is that
;; you should not assume everything behaves like a procedure call just because
